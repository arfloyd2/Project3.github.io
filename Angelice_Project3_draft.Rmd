...
logdta <- initial_train %>% dplyr::select(!c(Education_Level))
...
logloss3 <- train(Diabetes_binary~.,
                  data = logdta,
                  method = "glmStepAIC",
                  direction = "forward",
...
```{r loglosscomp}
model_method <- c("logLoss1_full_Model","logLoss2_Forward","Accuracy_Forward")
...
d the AIC is close to the lowest amount which was from the second logloss model. 
...
rrdta <- logdta %>% mutate(across(where(is.factor), as.character)) %>% mutate(across(where(is.character), as.numeric))

rrtest <- logdta %>% mutate(across(where(is.factor), as.character)) %>% mutate(across(where(is.character), as.numeric))
...
logbinmodel <- glm(Diabetes_binary~ PhysActivity + AnyHealthcare + HighBP +
          Smoker + Fruits + NoDocbcCost + Income + DiffWalk + HighChol + Stroke +
          Veggies + GenHlth + Sex + CholCheck + HeartDiseaseorAttack + HvyAlcoholConsump, data=initial_train1, 
