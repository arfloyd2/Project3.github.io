---
title: "Project3 Education Level: `r params$edlvl` "
...
Here are all the packages used throughout this document:
```{r libraries}
...
Because this data is in binary form, contingency tables and categorical visuals will be helpful since it will show how the different levels of those with and without diabetes compare to each other over levels of other categorical variables, as well as how values for the numerical variables compare at different levels for those with or without diabetes. 

## Diabetes, Health Measures and Pre-Existing Conditions 

It is often discussed that diet, and weight-related health issues such as obesity, high blood pressure and cholesterol are major contributing factors to complications with and obtaining diabetes. When the levels for these variabble are consistently high, there is often the observation that the chances for having diabetes later in life incerease. Thus, it would be interesting to see how the the levels of these variables in the data interact with patients who have diabetes - and in some cases, compared to the patients who do not have diabetes. 

## Diabetes Patients and Pre-Existing Conditions 

This frequency table will show the number of patients with and without diabetes who have (or do not have) high blood pressure, high cholesterol, have had stroke or have had a heart attack or heart disease.  
...
In this section, we will explore the distributions and central measures for BMI in patients with Diabetes

First we will observe the mean, median and maximum of BMI for patients with and without diabetes


This section will look at some levels of center for BMI. 
...
newcols <- dummyVars(~BMI + MentHlth + PhysHlth + Age + Education, data = initial_train, stringAsFactors = TRUE, sep = "_") %>% 
  predict(newdata = initial_train)

logdta <- cbind(initial_train,newcols) %>% 
          dplyr::select(-BMI, -MentHlth, - PhysHlth, - Age, - Education,-Education_1,-Education_Level)
...
This next model will use the stepAIC function in the forward direction to find a logistic model, still using the log loss metric. 
...
# Backwards method takes a very long time I've found
# 
# 
...
The third model will use the StepAIC fuction in the forward direction where the metric is Accuracy

```{r logloss3, warning=FALSE, eval = FALSE}
...
c(logloss3$deviance,logloss3$aic,logloss3$coefficients)

```


The model chosen will be LogLoss3 as this has the lowest amount of residual deviance, and the AIC is close to the lowest amount which was from the second logloss model. 
...
```{r rndmfrst, eval = FALSE}
# First, it seems that we have to convert the explanatory variables to numeric 

rrdta <- initial_train %>% mutate(across(where(is.factor), as.character)) %>% mutate(across(where(is.character), as.numeric))

rrtest <- initial_test %>% mutate(across(where(is.factor), as.character)) %>% mutate(across(where(is.character), as.numeric))

# Now fit the model 
rfFit <- randomForest(as.factor(Diabetes_binary)~., data = rrdta,mtry = round(sqrt(ncol(rrdta)),0), ntree = 200, importance = TRUE)
