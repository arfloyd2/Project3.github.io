...
Below reads in the binary heart disease data, converts the appropriate variables into factor level variables, splits the data into a training and test set, and pre-processes the data by centering and scaling.
...
```{r}
as.data.frame(table(filtered_dta$Diabetes_binary, filtered_dta$Sex)) %>% rename(Diabetes_binary="Var1", Sex = "Var2", Sex_Freq = "Freq")
```
...
```{r}
ggplot(filtered_dta, aes(x=as.factor(Diabetes_binary), y=BMI,
                        fill=as.factor(Sex))) +
  geom_boxplot() +
  labs(x = "Diabetes", y = "Body Mass Index (BMI)", title = "BMI of Those With and Without Diabetes by Sex")+
  scale_fill_discrete(name="Sex")

filtered_dta %>% group_by(Diabetes_binary, Sex) %>% summarize(BMI_mean = mean(BMI), BMI_sd = sd(BMI), BMI_Q1 = quantile(BMI, probs =0.25), BMI_Median = quantile(BMI, probs =0.5), BMI_Q3 = quantile(BMI, probs =0.75))
```

```{r}
ggplot(filtered_dta, aes(x=as.factor(Diabetes_binary), y=MentHlth,
                        fill=as.factor(Sex))) +
  geom_boxplot() +
  labs(x = "Diabetes", y = "Days of Poor Mental Health in Past 30", title = "Mental Health of Those With and Without Diabetes by Sex")+
  scale_fill_discrete(name="Sex")

filtered_dta %>% group_by(Diabetes_binary, Sex) %>% summarize(MentHlth_mean = mean(MentHlth), MentHlth_sd = sd(MentHlth), MentHlth_Q1 = quantile(MentHlth, probs =0.25), MentHlth_Median = quantile(MentHlth, probs =0.5), MentHlth_Q3 = quantile(MentHlth, probs =0.75))
```

```{r}
ggplot(filtered_dta, aes(x=as.factor(Diabetes_binary), y=PhysHlth,
                        fill=as.factor(Sex))) +
  geom_boxplot() +
  labs(x = "Diabetes", y = "Days of Illness/Injury in Past 30", title = "Physical Health of Those With and Without Diabetes by Sex")+
  scale_fill_discrete(name="Sex")

filtered_dta %>% group_by(Diabetes_binary, Sex) %>% summarize(PhysHlth_mean = mean(PhysHlth), PhysHlth_sd = sd(PhysHlth), PhysHlth_Q1 = quantile(PhysHlth, probs =0.25), PhysHlth_Median = quantile(PhysHlth, probs =0.5), PhysHlth_Q3 = quantile(PhysHlth, probs =0.75))
```
...
BP <- data.frame(table(filtered_dta$Diabetes_binary,filtered_dta$HighBP)) %>% rename(Diabetes_binary="Var1", High_BP = "Var2", BP_Freq = "Freq")
...
```{r diab_bmi}
filtered_dta %>% summarise(Max_BMI = max(BMI), Mediam_BMI = median(BMI), Mean_BMI = mean(BMI), .by = Diabetes_binary)
...
```{r graphage}
diab_bmi <- filtered_dta %>% filter(Diabetes_binary == "Yes")
...
PHYS <- data.frame(table(filtered_dta$Age,filtered_dta$PhysActivity)) %>% rename(Age_Group="Var1", Physical_Acitivty = "Var2", Phys_Freq = "Freq")
...
initial_age_group_diab <- filtered_dta %>% filter(Diabetes_binary == "Yes")
...
```{r mentalage}
ment_age <- filtered_dta %>% group_by(Diabetes_binary, Age) %>% summarize(mental_health_sum = sum(as.numeric(MentHlth)))
...
The below code creates a linear logistic model using all terms with no interaction or polynomial terms.
```{r Linear_Model, warning = FALSE}

Linear_Model <- train(Diabetes_binary ~ ., data =  trainTransformed,
                       method = "glm",
                       family = "binomial", 
                       trControl = ctrl,
                      metric = "logLoss",
...
predict_Linear_Model <- predict(Linear_Model, newdata = testTransformed,
                                type="prob")
...
Logloss_Linear <- mnLogLoss(data = test_Linear_Model,
                            lev = levels(test_Linear_Model$obs))
```

This next model will include all variables as well as an interaction between Sex and Age.
```{r Interaction Model, warning = FALSE}

Inter_Model <- train(Diabetes_binary ~ . + Sex:Age, data =  testTransformed,
                       method = "glm",
                       family = "binomial",
                       trControl = ctrl,
                      metric = "logLoss",
...
predict_Inter_Model <- predict(Inter_Model, newdata = testTransformed,
                                     type="prob")
...
Logloss_Inter <- mnLogLoss(data = test_Inter_Model,
                            lev = levels(test_Inter_Model$obs))
```

The third model will use the same interactions, but will also include the squared term for mental health.
```{r Polynomial, warning = FALSE}

Poly_Model <- train(Diabetes_binary ~ . + Sex:Age + I(MentHlth^2), data =  initial_train,
                       method = "glm",
                       family = "binomial",
                       trControl = ctrl,
                      metric = "logLoss",
...
predict_Poly_Model <- predict(Poly_Model, newdata = testTransformed,
                                     type="prob")
...
Logloss_Poly <- mnLogLoss(data = test_Poly_Model,
                            lev = levels(test_Poly_Model$obs))
```


We will now compare the three log-loss metrics from the test set to see which performs the best.
```{r loglosscomp}
Log_models <- data.frame(Model=c("Linear Model", "Interaction Model", "Polynomial Model"),
           LogLoss=c(Logloss_Linear, Logloss_Inter, Logloss_Poly)) %>%
  arrange(LogLoss)

Best_Log_Model <- Log_models[1,1]
```
The highest performing Log model is the `r Best_Log_Model`.
...
predict_LASSO_Model <- predict(LASSO_Model, newdata = testTransformed,
                                     type="prob")
...
Logloss_LASSO <- mnLogLoss(data = test_LASSO_Model,
                            lev = levels(test_LASSO_Model$obs))
```
...
predict_Class_Tree <- predict(Class_Tree, newdata = testTransformed,
                                     type="prob")
...
predict_random_forest <- predict(random_forest_model, newdata = testTransformed,
                                     type="prob")
...
Logloss_random_forest <- mnLogLoss(data = test_random_forest,
                            lev = levels(test_random_forest$obs))
```
...
logdta_train <- trainTransformed %>% dplyr::select(!c(MentHlth,PhysHlth,BMI))
...
predict_Neural_Model <- predict(Neural_Model, newdata = testTransformed,
                                     type="prob")
...
Logloss_Neural_Model <- mnLogLoss(data = test_Neural_Model,
                            lev = levels(test_Neural_Model$obs))
```
...
predict_XGBoost_Model <- predict(XGBoost_Model, newdata = testTransformed,
                                     type="prob")
...
Logloss_XGBoost_Model <- mnLogLoss(data = test_XGBoost_Model,
                            lev = levels(test_XGBoost_Model$obs))
```

# Best Overall Model

The below code selects the best overall model based on the log-loss metric of the test dataset.
```{r}
All_models <- data.frame(Model=c("Linear Model", "Interaction Model",
...
```
The highest performing overall model was the `r Best_Model`.
