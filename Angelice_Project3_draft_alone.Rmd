...
for(edlvl in c(1,3)) {rmarkdown::render(input = "Angelice_Project3_draft.Rmd",
              output_format = "github_document", 
              output_file = paste0("Project3","Education level:",edlvl,".md"),
              output_options = list(
...
diabage <- initial_dta %>% filter(Diabetes_binary == 1)
...
factordta <- lapply(initial_train[cols],factor)
...
logdta <- cbind(initial_train,newcols) %>% 
          dplyr::select(-BMI, -MentHlth, - PhysHlth, - Age, - Education, -Education1)
...
#Plotting the Results I think the varImPlot, might be the one to use, but there is also the top plot that we can use. Feel free to point out which one you lik better. 

plot(rfFit, log="y", main = "Random Forest Plot")

varImpPlot(rfFit, main = "Random Forest Plot")
